...
- [A single static Go binary, no runtime to start. The workspace walk runs in parallel, embeds are linted once, and `check` is built for the hot path — roughly 3x faster than Node markdownlint, with a CI gate against regression.](docs/features/performance.md)
