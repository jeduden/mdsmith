...
- [mdsmith collects no telemetry, no usage analytics, no error reports, and no identifiers. In the default configuration, the CLI and LSP make zero outbound network calls. MDS072 external-link-check is the one opt-in exception.](docs/reference/telemetry.md)
