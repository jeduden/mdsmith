...
func TestPrepareExportFile_BasicWiring(t *testing.T) {
	...
	f, rules, err := prepareExportFile(path, []byte(src), minimalConfig(), "", bytelimit.DefaultMaxInputBytes)
	require.NoError(t, err)
...
func TestPrepareExportFile_InvalidFrontMatterKinds_ReturnsError(t *testing.T) {
	...
	_, _, err := prepareExportFile(path, []byte(src), minimalConfig(), "", bytelimit.DefaultMaxInputBytes)
	require.Error(t, err)
	assert.True(t,
...
func TestPrepareExportFile_ConfigureRuleError_BubblesUp(t *testing.T) {
	...
	_, _, err = prepareExportFile(srcPath, src, cfg, cfgPath, bytelimit.DefaultMaxInputBytes)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "emphasis-style")
}
...
func TestDoExport_ReadError_ExitsTwo(t *testing.T) {
	...
	// bytelimit.ReadFileLimited fails and the CLI exits 2 with a message
	// on stderr.
...
func TestPrepareExportFile_NoCfgPath_UsesDocDirForGitignore(t *testing.T) {
	...
	f, _, err := prepareExportFile(path, []byte(src), minimalConfig(), "", bytelimit.DefaultMaxInputBytes)
	require.NoError(t, err)
