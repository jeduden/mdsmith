...
func TestDefaultSettings(t *testing.T) {
	...
	// archetype and archetype-roots are no longer supported settings.
	if _, ok := ds["archetype"]; ok {
		t.Errorf("archetype should not be in DefaultSettings")
	}
	if _, ok := ds["archetype-roots"]; ok {
		t.Errorf("archetype-roots should not be in DefaultSettings")
	}
}
...
func TestApplySettings_ArchetypeRootsProducesError(t *testing.T) {
	r := &Rule{}
	err := r.ApplySettings(map[string]any{"archetype-roots": []any{"dir"}})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "no longer supported")
}
...
func TestCheck_SchemaFileSuppressesRequireWarning(t *testing.T) {
	root := t.TempDir()
	// The schema file contains <?require?> — when linting the schema
	// itself, no warning should be emitted.
	schemaDir := filepath.Join(root, "schemas")
	require.NoError(t, os.MkdirAll(schemaDir, 0o755))
	require.NoError(t, os.WriteFile(
		filepath.Join(schemaDir, "story.md"),
		[]byte("<?require\nfilename: \"story-*.md\"\n?>\n# ?\n"),
		0o644))
	f := newFileInRoot(t, root, filepath.Join("schemas", "story.md"),
		"<?require\nfilename: \"story-*.md\"\n?>\n# ?\n")
	r := &Rule{Schema: filepath.Join("schemas", "story.md")}
	diags := r.Check(f)
	for _, d := range diags {
...
func TestCheck_NonSchemaFileWarnsMisplacedRequire(t *testing.T) {
	root := t.TempDir()
	src := "<?require\nfilename: \"doc-*.md\"\n?>\n# Title\n"
	f := newFileInRoot(t, root, "doc.md", src)
	// No schema configured — the file is a normal doc, so <?require?>
...
// isSchemaFile: non-schema file returns false
func TestIsSchemaFile_NonSchemaFile(t *testing.T) {
	r := &Rule{Schema: "schemas/story.md"}
	f := &lint.File{Path: "docs/other.md"}
	assert.False(t, r.isSchemaFile(f))
}

// isSchemaFile: schema file returns true
func TestIsSchemaFile_SchemaFileReturnsTrue(t *testing.T) {
	root := t.TempDir()
...
func TestIsSchemaFile_EmptySchemaReturnsFalse(t *testing.T) {
	r := &Rule{}
