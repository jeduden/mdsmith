		...
		case "archetype", "archetype-roots":
			return fmt.Errorf(
				"required-structure: setting %q is no longer supported; "+
					"use `schema:` with an explicit file path instead", k)
		case "placeholders":
...
// isSchemaFile reports whether f is the configured schema file. Schema
// files are treated as schema sources: <?require?> directives in them
// are meaningful, and the rule skips structural validation against themselves.
func (r *Rule) isSchemaFile(f *lint.File) bool {
	return r.Schema != "" && isSchemaFile(f.Path, r.Schema)
}
