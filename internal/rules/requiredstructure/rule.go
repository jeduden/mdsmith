		...
		case "archetype", "archetype-roots":
			return fmt.Errorf(
				"required-structure: setting %q is no longer supported; "+
					"use schema: <path> to reference a schema file directly, "+
					"or declare a kind with schema: set and assign it via kinds:/kind-assignment",
				k,
			)
		case "placeholders":
...
// Schema files are treated as schema sources: <?require?> directives
// in them are meaningful, and the rule skips structural validation
// against themselves.
