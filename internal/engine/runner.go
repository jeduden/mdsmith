...
//
// The block-only spike flag is mutually exclusive with the gate: it forces
// its own constructor, so the gate stands down when it is set.
func (r *Runner) layer0SkipEligible(
	source []byte, rules []rule.Rule, effective map[string]config.RuleCfg,
) bool {
	if !layer0SkipEnabled() || r.BlockOnlyParse {
		return false
	}
