...
func TestDefaultMaxInputBytes(t *testing.T) {
	assert.Equal(t, int64(2*1024*1024), readlimit.DefaultMaxInputBytes)
}
...
func TestReadFileLimited_Normal(t *testing.T) {
	...
	data, err := readlimit.ReadFileLimited(p, 100)
...
func TestReadFileLimited_AtLimit(t *testing.T) {
	...
	data, err := readlimit.ReadFileLimited(p, 50)
	require.NoError(t, err)
...
func TestReadFileLimited_OverLimit(t *testing.T) {
	...
	_, err := readlimit.ReadFileLimited(p, 50)
	require.Error(t, err)
...
func TestReadFileLimited_ZeroUnlimited(t *testing.T) {
	...
	data, err := readlimit.ReadFileLimited(p, 0)
	require.NoError(t, err)
...
func TestReadFileLimited_NegativeUnlimited(t *testing.T) {
	...
	data, err := readlimit.ReadFileLimited(p, -1)
	require.NoError(t, err)
	assert.Len(t, data, 10000)
}
...
func TestReadFileLimited_NotFound(t *testing.T) {
	_, err := readlimit.ReadFileLimited("/nonexistent/file.md", 100)
	require.Error(t, err)
}
...
func TestReadFileLimited_MaxInt64Unlimited(t *testing.T) {
	...
	data, err := readlimit.ReadFileLimited(p, math.MaxInt64)
	require.NoError(t, err)
	assert.Equal(t, []byte("hello"), data)
}
...
func TestReadFSFileLimited_MaxInt64Unlimited(t *testing.T) {
	...
	data, err := readlimit.ReadFSFileLimited(fsys, "test.md", math.MaxInt64)
	require.NoError(t, err)
	assert.Equal(t, []byte("hello"), data)
}
...
func TestReadFSFileLimited_Normal(t *testing.T) {
	...
	data, err := readlimit.ReadFSFileLimited(fsys, "test.md", 100)
	require.NoError(t, err)
	assert.Equal(t, []byte("hello"), data)
}
...
func TestReadFSFileLimited_OverLimit(t *testing.T) {
	...
	_, err := readlimit.ReadFSFileLimited(fsys, "big.md", 50)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "file too large")
}
...
func TestReadFSFileLimited_ZeroUnlimited(t *testing.T) {
	...
	data, err := readlimit.ReadFSFileLimited(fsys, "big.md", 0)
	require.NoError(t, err)
	assert.Len(t, data, 10000)
}
...
func TestReadFSFileLimited_AtLimit(t *testing.T) {
	...
	data, err := readlimit.ReadFSFileLimited(fsys, "exact.md", 50)
	require.NoError(t, err)
	assert.Equal(t, content, data)
}
...
func TestReadFSFileLimited_Nonexistent(t *testing.T) {
	fsys := fstest.MapFS{}
	_, err := readlimit.ReadFSFileLimited(fsys, "no-such.md", 100)
	require.Error(t, err)
}
