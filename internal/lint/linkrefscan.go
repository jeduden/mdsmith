...
// refDefMarker (declared in inline_blocks.go, value `]:`) is the minimal
// byte sequence every link reference definition contains: a `]`
// immediately followed by `:`. A paragraph whose head holds no `]:`
// cannot open a definition, so the scanner skips it without building any
// segments. A block quote or list line holding `]:` is the fallback
// trigger (see scanNeedsFallback).

// scanLinkReferences returns the link reference definitions in f by
// scanning the head of every Layer 0 paragraph (and setext-heading)
// block, without a full goldmark document parse. It mirrors goldmark's
// first-wins dedup: the first definition for a normalised label
// survives, so blocks are fed to the shared parser scanner in document
// order.
//
// The scanner only descends into top-level (Depth 0) paragraph and
// setext-heading blocks. Definitions nested in a block quote or list
// item are real to goldmark but invisible here, so a caller that needs
// full coverage must consult scanNeedsFallback first and parse the
// document when it returns true. scanLinkReferences itself returns
// whatever the block heads yield; LinkReferences applies the fallback
// gate.
func scanLinkReferences(f *File) []Reference {
	...
	for _, span := range l0.BlockSpans {
		// A depth-0 paragraph is the common home of a definition. A
		// depth-0 setext-heading span is the other: goldmark peels a
		// leading `[label]: dest` definition off the run *before* the
		// setext underline turns the rest into a heading, so a definition
		// directly above a `===`/`---` underline is real to the full parse
		// (e.g. `[a]: /a` then `===`). Layer 0 flattens that pair into one
		// BlockSetextHeading span, so feed its head to the parser too —
		// parseLinkReferenceDefinition consumes the definition lines and
		// stops at the underline, matching the AST.
		if span.Depth != 0 || (span.Kind != BlockParagraph && span.Kind != BlockSetextHeading) {
			continue
		}
