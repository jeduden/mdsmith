...
func TestToTextEdits(t *testing.T) {
	t.Parallel()
	got := toTextEdits([]refactor.Edit{
		{Range: refactor.Range{
			Start: refactor.Position{Line: 1, Character: 2},
			End:   refactor.Position{Line: 1, Character: 7},
		}, NewText: "x"},
	})
	require.Len(t, got, 1)
...
func TestToLSPChanges(t *testing.T) {
	...
	got := toLSPChanges(map[string][]refactor.Edit{})
	assert.NotNil(t, got)
	assert.Empty(t, got)
...
func TestServer_writeRenameError(t *testing.T) {
	...
	t.Run("heading collision carries conflict data", func(t *testing.T) {
		...
		s.writeRenameError(json.RawMessage(`1`), refactor.HeadingCollisionError{Conflict: "Intro"})
		out := buf.String()
	...
	t.Run("label collision carries conflict data", func(t *testing.T) {
		var buf safeBuffer
		s := New(Options{Reader: nil, Writer: &buf})
		s.writeRenameError(json.RawMessage(`1`), refactor.LabelConflictError{Conflict: "Docs"})
		out := buf.String()
		assert.Contains(t, out, `"code":-32602`)
	...
	t.Run("plain typed error surfaces its message", func(t *testing.T) {
		var buf safeBuffer
		s := New(Options{Reader: nil, Writer: &buf})
		s.writeRenameError(json.RawMessage(`1`), refactor.ErrEmptyHeadingSlug)
		out := buf.String()
		assert.Contains(t, out, `"code":-32602`)
		assert.Contains(t, out, refactor.ErrEmptyHeadingSlug.Error())
	})
}
