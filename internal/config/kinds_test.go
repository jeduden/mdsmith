...
func TestMergeKindSettingsIsolatedFromLoaded(t *testing.T) {
	// Verify that Merge does not share Settings map references between
	// the loaded and merged configs, so mutations to one don't affect the other.
	loaded := &Config{
		Kinds: map[string]KindBody{
			"plan": {Rules: map[string]RuleCfg{
	...
	// Mutate merged config's Settings; loaded must not be affected.
	mergedRC := merged.Kinds["plan"].Rules["required-structure"]
	mergedRC.Settings = map[string]any{"schema": "plan/schema.md"}
	merged.Kinds["plan"] = KindBody{Rules: map[string]RuleCfg{
		"required-structure": mergedRC,
	}}

	assert.Nil(t, loaded.Kinds["plan"].Rules["required-structure"].Settings,
		"mutating merged config's Settings must not affect loaded config")
}
