...
// comment, wraps prose to fragmentWrapWidth so the repository
// line-length rule (MDS001) passes, and is consumed verbatim
// by <?include?> in README intros.
type MarkdownFragment struct{}

// fragmentHeader is split across two lines so the longer one
// fits inside fragmentWrapWidth. The form matches the
// precedent set by docs/research/benchmarks/headline.fragment.md.
const fragmentHeader = "<!-- Generated by `mdsmith-release sync-messaging` from\n" +
	"docs/brand/messaging.md — do not edit by hand. -->"

// fragmentWrapWidth keeps both the header and the body inside
// the repository's MDS001 line-length cap (.mdsmith.yml sets
// max: 80). Word-wrap at 75 leaves a small margin for the
// occasional wide token (em-dash + word).
const fragmentWrapWidth = 75

// ReadValue returns the fragment body with the header comment
// and the trailing blank line stripped. Returns an error when
// the canonical header is missing — a hand-edited or
// non-fragment file must not silently pass the drift check.
func (MarkdownFragment) ReadValue(body []byte) (string, error) {
	s := string(body)
	if !strings.HasPrefix(s, fragmentHeader+"\n") {
		return "", errors.New(
			"markdown fragment is missing the canonical header comment")
	}
	s = strings.TrimPrefix(s, fragmentHeader+"\n")
	// PatchValue inserts a blank line between the header and
	// the wrapped body. Drop it on the way back and rejoin
	// wrapped lines so the round-trip preserves the caller's
	// value.
...
// blank line, word-wrapped value at fragmentWrapWidth, trailing
// newline.
func (MarkdownFragment) PatchValue(_ []byte, value string) ([]byte, error) {
	value = strings.TrimRight(value, "\n")
	wrapped := wordwrap.WrapString(value, fragmentWrapWidth)
	return []byte(fragmentHeader + "\n\n" + wrapped + "\n"), nil
}

// unwrap joins single-line-break wrapped text back into one
// long line per paragraph (paragraphs separated by blank
// lines). It is the inverse of word-wrap for ReadValue's
// round-trip contract.
