...
2. Define the `File` type (`internal/lint/file.go`):
   `Path string`, `Source []byte`, `Lines [][]byte`,
   `AST ast.Node`. Constructor parses source with goldmark
   once; all rules share the result.
3. Define the `Diagnostic` type
   (`internal/lint/diagnostic.go`): fields `File`, `Line`,
   `Column`, `RuleID`, `RuleName`, `Severity`, `Message`.
...
- [ ] `File` constructor accepts raw bytes, returns a struct
      with parsed AST (goldmark `ast.Document`) and `Lines`
      split correctly
- [ ] Parsing an empty file produces a valid (empty) AST without error
- [ ] Parsing a file with headings, code blocks, and lists
      produces a non-nil AST
