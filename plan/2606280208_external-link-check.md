...
Close the gap with gomarklint's `external-link` rule by adding
`MDS071 external-link-check` — a default-off rule that HEAD-checks every
`http://` and `https://` URL in the workspace, caches per-URL results across
files for the duration of the run, and emits a diagnostic for each URL that
returns an error, a 4xx, or a 5xx response. Resolves
...
The shared `links:` config block already carries `external-skip` (a
`[]string` of regex patterns) in `internal/rules/linkstyle.LinksConfig`, with
tests in `rule_test.go` at line 268. MDS071 reads that same key and two new
ones (`external-timeout`, `external-rate-limit`) from the shared `links:` block.
