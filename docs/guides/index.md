...
| [Open Knowledge Format (OKF) bundles](okf.md)                                       | Author and validate Open Knowledge Format bundles with mdsmith: pin the `okf` convention to require a non-empty `type` on every concept document, validate bundle-relative cross-links with `links.site-root`, and generate the `index.md` listing with `<?catalog?>`.                                            |
<?/catalog?>
