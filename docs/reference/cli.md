...
| [`init`](cli/init.md)                         | Generate a default `.mdsmith.yml` config in the current directory, scaffold a workflow config with `--starter`, or convert an existing markdownlint config with `--from-markdownlint`. |
